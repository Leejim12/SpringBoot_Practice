...
public class LogDaoH2Impl extends ConnectH2 implements LogDAO {
	String drv = "org.h2.Driver";
	String url = "jdbc:h2:tcp://localhost/~/test";  
	String id = "sa";
	String pwd = "";
	
	public LogDaoH2Impl(String drv, String url, String id, String pwd) {
	...
	public List<LogVO> getLogVO() {
		...
		try {
			...
			while(rs.next()) {
				...
				dto.setLogContent(rs.getString("LogContent"));
				ls.add(dto);
			}
		}catch(Exception e) {
			e.printStackTrace();
	...
	public int InsertLog(LogVO vo) {
		...
		try {
			String query = "INSERT INTO LOG("
					...
					+ "?)";
			psmt = con.prepareStatement(query);
			psmt.setString(2, vo.getLogContent());
	...
	public int UpdateLog(LogVO vo) {
		...
		try {
			...
			psmt.setString(1, vo.getLogContent());
			result = psmt.executeUpdate();
		} catch(Exception e) {
